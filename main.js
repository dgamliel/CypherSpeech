...
handshake.createSharedKey = function (receivedKey) {
	...
	.then(sharedSecret => {
		...
		console.log("In Function: Create Shared Key return", secret);
		return secret;
	})
	.catch(err => {
		console.error(err);
...
$(function () {
	...
	socket.on('pubFromPeer', async data => {
		let remotePub = await handshake.genEcdhKeyPair();
		let receivedKey = data;
		var shared = await handshake.createSharedKey(receivedKey);

		console.log("in pubFromPeer key generated is ", remotePub);
		console.log('Creating shared key from peer', shared);
		socket.emit('pubFromRemote', remotePub);
...
async function establishEncryption(socket){
	...
	var pub = await handshake.genEcdhKeyPair()
	console.log('keyPair generated by peer is', pub);
	socket.emit('pubFromPeer', pub);
